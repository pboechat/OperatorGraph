...
<code>
	...
	struct Controller : Scenes::GriddedScene < <gridY>, <gridX> >
	{
		...
		struct AxiomTraits : Grid::DefaultAxiomTraits < Box, 0 >
		{
			...
			{
				return math::float3(25.0f, 50.0f, 25.0f);
			}
		...
		struct TerminalsTraits
		{
			...
			{
				math::float2 vertices[3] = { { -0.5, -0.5 }, { 0.5, 0 }, { -0.5, 0.5 } };
				switch (terminalIndex)
				{
				case 0:
				...
				case 2:
					return std::unique_ptr<InstanceMesh>(new ShapeMesh<DPrism>(DPrism(vertices, 3)));
