		...
		namespace GL
		{
			...
			protected:
					...
					)
				{
					...
					size_t c = mesh->appendVertexAttributes(positions, normals, uvs);
					if (c == 0)
						return;
					mesh->appendIndices(indices, offset);
			...
			public:
					...
					)
				{
					...
					exportToOBJ(objOut, mtlOut, name, "", materials, useLeftHandedCoordsSystem);
				}
				...
				virtual PGA::Rendering::GL::Mesh* getMeshPtr(size_t i) const = 0;
